...
class Home extends CI_Controller
{
	...
	public function more_products($id='')
	{
		...
		$data['products']=$this->cm->select_data('products','*');
        $data['all_category']=$this->cm->select_data('product_category','*',array('status'=>'1'));
         
		$this->load->view('includes/header');
		$this->load->view('more_products',$data);
		$this->load->view('includes/footer');
	}
	public function more_brand($id='')
	{
		if($id!='')
		{
		...
		$data['products']=$this->cm->select_data('products','*');
        $data['all_brand']=$this->cm->select_data('product_brand','*',array('status'=>'1'));
         
		$this->load->view('includes/header');
		$this->load->view('more_brand',$data);
		$this->load->view('includes/footer');
	}
	...
	public function signup()
	{
		if($this->input->method()=='post')
		{
			...
			{
				$resp=$this->cm->select_data('users','*',array('email'=>$_POST['username']));
				if($resp)
				{
				...
				else
				{
			       ...
			       $_POST['password']=md5($_POST['password']);
					...
					$res=$this->cm->insert_data('users',$_POST);
					if($res)
						$arr=array('status'=>'true','message'=>'Success','reload'=>base_url().'index.php/home/login');
					else
	...
	public function cod_payment()
	{
			...
			$arr=array('user_info'=>json_encode($user_info),'product_info'=>json_encode($product_info),'payment_mode'=>'Payment Pending','status'=>'0','user_id'=>$user_session['user_id']);
			$this->cm->insert_data('order_master',$arr);
			$this->cm->update_data('add_to_cart',array('status'=>'0'),array('add_to_cart.status'=>'1','add_to_cart.user_id'=>$user_session['user_id']));
			redirect(base_url().'index.php/home/success');
	}
